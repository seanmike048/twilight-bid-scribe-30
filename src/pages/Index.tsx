...
import { runAnalysis, AnalysisEntry } from "@/lib/runAnalysis";
...
const ExamplesDropdown = ({
  ...
  onLoadExample: (key: keyof typeof exampleBidRequests) => void;
}) => (
  <DropdownMenu>
    ...
    <DropdownMenuContent>
      <DropdownMenuItem onSelect={() => onLoadExample("display")}>
        Display Banner
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onLoadExample("video")}>
        Video Ad
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onLoadExample("native")}>
        Native Ad
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onLoadExample("ctv")}>
        Connected TV
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onLoadExample("audio")}>
        Audio Ad
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onLoadExample("gdpr")}>
        GDPR Example
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onLoadExample("error-privacy")}>
        Privacy Error
      </DropdownMenuItem>
...
export default function IndexPage() {
  ...
  const [multiResults, setMultiResults] = useState<AnalysisEntry[]>([]);
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    setTimeout(() => {
      const results = runAnalysis(jsonText);
      if (!results.length) {
      ...
      if (analysis && !analysis.error) {
        try {
          ...
          if (
            ...
            parsedRequest.source.schain &&
            Array.isArray(parsedRequest.source.schain.nodes)
          ) {
  ...
  const handleLoadExample = useCallback(
    (key: keyof typeof exampleBidRequests) => {
  ...
  const handleFileUpload = useCallback((file: File) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setIsLoading(false);
        toast.error(
          "Failed to parse file. Ensure it contains valid JSON objects.",
        );
      }
    };
  ...
  const handleRequestSelection = useCallback((request: any) => {
    ...
    setTimeout(() => {
      const { analysis, issues } = analyzer.analyze(
